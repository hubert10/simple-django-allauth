...
class CustomProcessAdapter(DefaultAccountAdapter):
    ...
    def save_user(self, request, user, form, commit=True):
        # Any action regarding user object can be performed here!
        user = super(CustomProcessAdapter, self).save_user(request, user, form)
        return user
...
class SocialAccountAdapter(DefaultSocialAccountAdapter):
    def pre_social_login(self, request, sociallogin):
        ...
        try:
            ...
            email = sociallogin.account.extra_data['elements'][0]['handle~']['emailAddress'].lower(
            )
